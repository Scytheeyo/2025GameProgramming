...
public class CloverProjectile : MonoBehaviour
{
    ...
    private Vector2 savedVelocity; // 멈추기 직전 속도 저장용
    private bool isFrozen = false; // 현재 멈춰있는지 확인용
    private Rigidbody2D rb;

    [Header("Effects")]
    public GameObject hitEffectPrefab; // 피격 이펙트 프리팹

    void Awake() // Start 대신 Awake 권장 (GetComponent를 미리 하기 위해)
    {
    ...
    {
        if (isFrozen) return; // 이미 멈췄으면 중복 실행 방지
    ...
    IEnumerator FreezeRoutine(float duration)
    {
        ...
        if (rb != null)
        {
            ...
            rb.isKinematic = true; // 물리 연산 잠시 끄기 (밀림 방지)
        }
        ...
        if (rb != null)
        {
            rb.isKinematic = false; // 원래 설정이 Kinematic이었다면 true로 두세요
